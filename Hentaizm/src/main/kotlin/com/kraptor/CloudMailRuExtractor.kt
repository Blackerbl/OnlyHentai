...
open class CloudMailRu : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) = withContext(Dispatchers.IO) {
        val headers = mapOf(
            "Accept" to "*/*",
        ...
        callback.invoke(
            newExtractorLink(
                source  = this@CloudMailRu.name,
                name    = this@CloudMailRu.name,
                url     = videoUrl,
                type    = INFER_TYPE
            ) {
